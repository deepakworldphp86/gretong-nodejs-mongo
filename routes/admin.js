...
const app = express();
console.log(global.dbo);


/*************************** Model *********************************/

let Categories = require('../models/categories');
let Admin = require('../models/admin-user');
...
router.get('/categories/add/:id', auth.isAuthorized, function (request, response, next) {
...
router.post('/updatecategories', upload.single('categories_image'), function (req, res, next) {
    auth.isAuthorized;
    callback();
    ...
    req.checkBody('categories_description', 'Categories Description is required').notEmpty();
    req.checkBody('categories_content', 'Categories Content is not valid').notEmpty();
    ...
    if (errors || !categories_image) {
        ...
        Categories.findByIdAndUpdate(req.body.id, objectCat, {upsert: true}, function (err, num, n) {
            if (err) {
                throw err;
            } else {
...
router.post('/savecategories', upload.single('categories_image'), function (req, res, next) {
    auth.isAuthorized;
    const id = req.body.id;
    req.checkBody('categories', 'Categories Name is required').notEmpty();
    req.checkBody('categories_description', 'Categories Description is required').notEmpty();
...
router.get('/category/delete', function (req, res) {
    ...
    Categories.count({parent_category: id.trim()}).then(count => {
        customEvents.emit('categoryDeleteBefore', 'Count of child categorys' + count);
        if (count === 0) {
